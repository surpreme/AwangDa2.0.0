package com.aite.awangdalibrary.view;
...
public class LabelsView extends ViewGroup implements View.OnClickListener, View.OnLongClickListener {
    ...
    private ArrayList<Integer> mEnableLabels = new ArrayList<>();
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            mLabelWidth = mTypedArray.getLayoutDimension(R.styleable.labels_view_labelsTextWidth, mLabelWidth);
            mLabelHeight = mTypedArray.getLayoutDimension(R.styleable.labels_view_labelsTextHeight, mLabelHeight);

            if (mTypedArray.hasValue(R.styleable.labels_view_labelsTextColor)) {
                mTextColor = mTypedArray.getColorStateList(R.styleable.labels_view_labelsTextColor);
            } else {
                mTextColor = ColorStateList.valueOf(0xFF000000);
            }

            mTextSize = mTypedArray.getDimension(R.styleable.labels_view_labelsTextSize,
                    sp2px(14));
            if (mTypedArray.hasValue(R.styleable.labels_view_labelsTextPadding)) {
                int textPadding = mTypedArray.getDimensionPixelOffset(
                        R.styleable.labels_view_labelsTextPadding, 0);
                ...
                mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                        R.styleable.labels_view_labelsTextPaddingLeft, dp2px(10));
                mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                        R.styleable.labels_view_labelsTextPaddingTop, dp2px(5));
                mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                        R.styleable.labels_view_labelsTextPaddingRight, dp2px(10));
                mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                        R.styleable.labels_view_labelsTextPaddingBottom, dp2px(5));
            }
            ...
            if (mTypedArray.hasValue(R.styleable.labels_view_labelsBackground)) {
                int labelBgResId = mTypedArray.getResourceId(R.styleable.labels_view_labelsBackground, 0);
                if (labelBgResId != 0) {
                    mLabelBg = getResources().getDrawable(labelBgResId);
                } else {
                    int labelBgColor = mTypedArray.getColor(R.styleable.labels_view_labelsBackground, Color.TRANSPARENT);
...
//            测试的数据
    ...
    private static final String KEY_ENABLE_LABELS_STATE = "key_enable_labels_state";
...
//        }
        ...
        //保存不能选择的标签列表
        if (!mEnableLabels.isEmpty()) {
            bundle.putIntegerArrayList(KEY_ENABLE_LABELS_STATE, mEnableLabels);
        }
...
//            }
            ...
            ArrayList<Integer> enableLabels = bundle.getIntegerArrayList(KEY_ENABLE_LABELS_STATE);
            if (enableLabels != null && !enableLabels.isEmpty()){
                int size = enableLabels.size();
                int[] positions = new int[size];
                for (int i = 0; i < size; i++) {
                    positions[i] = enableLabels.get(i);
                }
                setSelects(positions);
            }
            return;
    ...
    private void setLabelEnable(TextView label, boolean isEnable) {
        /*if (label.isSelected() != isSelect) {
            label.setSelected(isSelect);
            if (isSelect) {
                mSelectLabels.add((Integer) label.getTag(KEY_POSITION));
        ...
        if (label.isSelected() && !isEnable){
            label.setSelected(false);
            mSelectLabels.remove((Integer) label.getTag(KEY_POSITION));
            if (mLabelSelectChangeListener != null) {
                mLabelSelectChangeListener.onLabelSelectChange(label, label.getTag(KEY_DATA),
                        false, (int) label.getTag(KEY_POSITION));
            }
        }
     ...
     * @param positions
     */
    public void setEnables(int... positions) {
        if (mSelectType != SelectType.NONE) {
            ArrayList<TextView> enableLabels = new ArrayList<>();
            int count = getChildCount();
            for (int p : positions) {
                if (p < count) {
                    TextView label = (TextView) getChildAt(p);
                    if (!enableLabels.contains(label)) {
                        setLabelEnable(label, false);
                        enableLabels.add(label);
                    }
                }
            }
