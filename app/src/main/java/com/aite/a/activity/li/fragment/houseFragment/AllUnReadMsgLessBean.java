package com.aite.a.activity.li.fragment.houseFragment;

/**
 * @author:lzy
 * @Date: 2019/12/20
 * @description:
 */
public class AllUnReadMsgLessBean {


    /**
     * newcommon : 0
     * newsystem : 71
     * newpersonal : 0
     * new_store_msg : 61
     * total_num : 132
     */

    private String newcommon;
    private String newsystem;
    private String newpersonal;
    private String new_store_msg;
    private String total_num;

    public String getNewcommon() {
        return newcommon;
    }

    public void setNewcommon(String newcommon) {
        this.newcommon = newcommon;
    }

    public String getNewsystem() {
        return newsystem;
    }

    public void setNewsystem(String newsystem) {
        this.newsystem = newsystem;
    }

    public String getNewpersonal() {
        return newpersonal;
    }

    public void setNewpersonal(String newpersonal) {
        this.newpersonal = newpersonal;
    }

    public String getNew_store_msg() {
        return new_store_msg;
    }

    public void setNew_store_msg(String new_store_msg) {
        this.new_store_msg = new_store_msg;
    }
    public String getTotal_num() {
        return total_num;
    }

    public void setTotal_num(String total_num) {
        this.total_num = total_num;
    }

}
