...
//        holder.goods_price.setText(activity.getString(R.string.zhifufangshi) + orderList.payment_name + "\t" + activity.getString(R.string.release_goods96));
        holder.goods_price.setText(activity.getString(R.string.zhifufangshi) + orderList.payment_name );
        holder.amount.setText(orderList.order_amount);
        holder.shippingFeeTv.setText(orderList.shipping_fee);
        if (!TextUtils.isEmpty(orderList.write_off_code)) {
            holder.writeOffCode_tv.setText(String.format(activity.getString(R.string.receivecode), orderList.write_off_code));
            holder.qr_iv.setOnClickListener(ddd -> {
                if (onQrCodeClickInterface != null)
                    onQrCodeClickInterface.onClick(orderList.write_off_code);


            });
        } else {
            holder.writeOffCode_tv.setText("");
            holder.qr_iv.setOnClickListener(ddd -> {
                if (onQrCodeClickInterface != null)
                    onQrCodeClickInterface.onClick(orderList.write_off_code);
        ...
        holder.qr_iv.setVisibility(TextUtils.isEmpty(orderList.write_off_code) ? View.GONE : View.VISIBLE);
...
//			String userId = State.UserId;
    ...
    public interface OnQrCodeClickInterface {
        void onClick(String code_number);
    }
...
//						Dialog(APPSingleton.getContext().getString(R.string.sure_cancel_order).toString(), v, what, orderList);
                    ...
                    case R.id.order_datails:
                        //订单详情
                        Intent intent = new Intent(activity, OrderInformationKotlinActivity.class);
//                        Intent intent = new Intent(activity, OrderInfoActivity.class);
                        intent.putExtra("order_id", orderList.order_id);
                        intent.putExtra("key", State.UserKey);
                        activity.startActivity(intent);
                        break;
                    case R.id.order_pay:
                        //订单支付
                    ...
                    case R.id.order_logistics:
                        //查看物流
                        activity.startActivity(new Intent(activity, LogisticsKotlinActivity.class).putExtra("order_id", orderList.order_id));
...
//                        activity.startActivity(intentwl);
    ...
    private void goodsPay(String pay_sn) {
        Intent intent = new Intent(activity, SurePayListActivity.class);
    ...
    private static class ViewHolder {
        ...
        public Button detele_bt, order_pay, qr_iv;
        public TextView amount;
        public TextView shippingFeeTv, writeOffCode_tv;
        ...
        public ViewHolder(View v) {
            super();
            qr_iv = (Button) v.findViewById(R.id.qr_iv);
            order_store = (TextView) v.findViewById(R.id.order_store);
            writeOffCode_tv = (TextView) v.findViewById(R.id.writeOffCode_tv);
