...
public class MvvmShoppingCartAdapter extends BaseAdapter {
    ...
    public MvvmShoppingCartAdapter(Context mcontext, List<ShoppingCardBean.CartListBean> cart_list, Handler handler) {
     ...
     * @param cart_list
     */
    public void refreshData(List<ShoppingCardBean.CartListBean> cart_list) {
    ...
    public void pickAll(boolean pick) {
        ...
        for (int i = 0; i < cart_list.size(); i++) {
            ...
            for (int j = 0; j < cart_list.get(i).getGoods_list().size(); j++) {
                cart_list.get(i).getGoods_list().get(j).ispick = pick;
            }
        }
        notifyDataSetChanged();
        getTotalPrice();
    ...
    public void pickStore(int id) {
        ...
        for (int i = 0; i < cart_list.get(id).getGoods_list().size(); i++) {
            cart_list.get(id).getGoods_list().get(i).ispick = cart_list.get(id).ispick;
        }
        notifyDataSetChanged();
        getTotalPrice();
    }
    ...
    public void getTotalPrice() {
        ...
        for (int i = 0; i < cart_list.size(); i++) {
            ...
            for (int j = 0; j < cart_list.get(i).getGoods_list().size(); j++) {
                if (cart_list.get(i).getGoods_list().get(j).ispick == false) isall = false;
                if (cart_list.get(i).getGoods_list().get(j).ispick) {
                    price += Float.parseFloat(cart_list.get(i).getGoods_list().get(j).getGoods_price()) * Float.parseFloat(cart_list.get(i).getGoods_list().get(j).getGoods_num());
                    num+=Float.parseFloat(cart_list.get(i).getGoods_list().get(j).getGoods_num());
                }
            }
        }
        ...
        map.put("num", num);
        map.put("isall", isall);
//        RxBus.getDefault().post(map, DataConstant.RxBusConstant.RXBUS_SHOPCARD_MSG);
//        Messenger.getDefault().send(map,DataConstant.TOKEN_SHOPCARD_MSG);
    ...
    public void setNumber(Map<String, String> map) {
        for (int i = 0; i < cart_list.size(); i++) {
            for (int j = 0; j < cart_list.get(i).getGoods_list().size(); j++) {
                if (cart_list.get(i).getGoods_list().get(j).getGoods_id().equals(goods_id)) {
                    cart_list.get(i).getGoods_list().get(j).setGoods_num(map.get("quantity"));
                    cart_list.get(i).getGoods_list().get(j).setGoods_price(map.get("goods_price"));
                    cart_list.get(i).getGoods_list().get(j).setGoods_sum(map.get("total_price"));
                    break;
                }
            }
        }
    ...
    public String getPickGoodsId() {
        ...
        for (int i = 0; i < cart_list.size(); i++) {
            for (int j = 0; j < cart_list.get(i).getGoods_list().size(); j++) {
                if (cart_list.get(i).getGoods_list().get(j).ispick) {
                    stringBuffer.append(cart_list.get(i).getGoods_list().get(j).getCart_id() + "|");
                }
            }
        }
    ...
    public String getPickGoodsId2() {
        ...
        for (int i = 0; i < cart_list.size(); i++) {
            for (int j = 0; j < cart_list.get(i).getGoods_list().size(); j++) {
                if (cart_list.get(i).getGoods_list().get(j).ispick) {
                    stringBuffer.append(cart_list.get(i).getGoods_list().get(j).getCart_id() + "|" + cart_list.get(i).getGoods_list().get(j).getGoods_num() + ",");
                }
            }
        }
        if (stringBuffer.toString().length() == 0) return null;
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        final ShoppingCardBean.CartListBean cart_list = this.cart_list.get(i);
        holder.iv_pick.setImageResource(cart_list.ispick ? R.drawable.check_red_2 : R.drawable.check_red_1);
        holder.tv_storename.setText(cart_list.getStore_name());
        GoodsAdapter adapter = new GoodsAdapter(cart_list.getGoods_list(), i);
        holder.lv_goods.setAdapter(adapter);
        holder.ll_tostore.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(mcontext,
                        ShopHomeActivity.class);
                Bundle bundle = new Bundle();
                bundle.putString("store_id", cart_list.getStore_id());
    ...
    private class GoodsAdapter extends BaseAdapter {
        ...
        public GoodsAdapter(List<ShoppingCardBean.CartListBean.GoodsListBean> goods_list, int id) {
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            ...
            final ShoppingCardBean.CartListBean.GoodsListBean goods_list = this.goods_list.get(i);
            holder.iv_pick.setImageResource(goods_list.ispick ? R.drawable.check_red_2 : R.drawable.check_red_1);
            Glide.with(mcontext).load(goods_list.getGoods_image_url()).into(holder.iv_img);
            holder.tv_goodsname.setText(goods_list.getGoods_name());
            holder.tv_price.setText("$" + goods_list.getGoods_price());
            holder.tv_number.setText(goods_list.getGoods_num());
            if (goods_list.getGoods_spec() == null || goods_list.getGoods_spec().length() == 0) {
                holder.tv_srec.setVisibility(View.INVISIBLE);
            } else {
                holder.tv_srec.setVisibility(View.VISIBLE);
                holder.tv_srec.setText(goods_list.getGoods_spec());
            }
            holder.tv_plus.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {//加
                    int i1 = Integer.parseInt(goods_list.getGoods_num());
                    i1++;
                    Map<String, String> map = new HashMap<>();
                    map.put("num", i1 + "");
                    map.put("cart_id", goods_list.getCart_id());
                    goods_id = goods_list.getGoods_id();
                    if (ClutterUtils.isInteger(goods_list.getGoods_price()))
                        holder.tv_price.setText(Integer.parseInt(goods_list.getGoods_price()) * i1);
...
//                    Intent intent = new Intent(mcontext, ProductDetailsActivity.class);
                    Intent intent = new Intent(mcontext, ProductDetailsKotlinActivity.class);
